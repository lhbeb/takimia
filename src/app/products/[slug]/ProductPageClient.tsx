    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center px-4">
          ...
          <Link
            href="/"
            className="inline-block bg-[#2e3868] hover:bg-[#1f274a] text-[#F0F6FF] px-6 py-3 rounded-lg transition-colors duration-300"
          >
            Return to Home
          </Link>
        </div>
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:items-start">
            <div className="relative lg:sticky lg:top-0 lg:self-start">
                  ...
                  {images.map((image, idx) => (
                    image ? (
                      <button
                        ...
                        className={`relative flex-shrink-0 w-20 h-20 rounded-md overflow-hidden ${activeImage === idx ? 'ring-2 ring-[#2e3868]' : 'ring-1 ring-gray-200'}`}
                      >
                        <Image
                          src={image}
              ...
              {images.length > 1 && (
                <>
                  <button onClick={() => setActiveImage((prev) => (prev > 0 ? prev - 1 : images.length - 1))} className="absolute left-4 top-1/2 -translate-y-1/2 transform bg-white/80 hover:bg-[#2e3868] hover:text-[#F0F6FF] p-2 rounded-full transition-all duration-300 z-10">
                    <ChevronLeft className="h-6 w-6" />
                  </button>
                  <button onClick={() => setActiveImage((prev) => (prev < images.length - 1 ? prev + 1 : 0))} className="absolute right-4 top-1/2 -translate-y-1/2 transform bg-white/80 hover:bg-[#2e3868] hover:text-[#F0F6FF] p-2 rounded-full transition-all duration-300 z-10">
            ...
            <div className="lg:pr-4">
              ...
              {condition && (
                <div className="mt-3 w-fit max-w-full">
                  ...
                  >
                    <div className="inline-flex max-w-full items-center gap-1.5 rounded-full border border-gray-200 bg-gray-100 px-3 py-1 text-sm font-medium text-gray-700 transition-colors group-hover:border-[#2e3868]/30 group-hover:bg-[#2e3868]/5 group-focus-within:border-[#2e3868]/30 group-focus-within:bg-[#2e3868]/5">
                      <span className="truncate">{getConditionDisplayLabel(condition)}</span>
                      <Info className="h-4 w-4 flex-shrink-0 text-gray-400 transition-colors group-hover:text-[#2e3868] group-focus-within:text-[#2e3868]" />
                    </div>
                    {getConditionTooltip(condition) && isConditionTooltipVisible && (
                      <div
                        className="pointer-events-none z-[70] w-72 max-w-[calc(100vw-2rem)] rounded-xl border border-[#2e3868]/10 bg-[#2e3868] px-3 py-2 text-xs leading-5 text-[#F0F6FF] shadow-xl"
                        style={conditionTooltipStyle}
                      >
                        {getConditionTooltip(condition)}
                        <div className="absolute bottom-full left-5 border-4 border-transparent border-b-[#2e3868] md:bottom-auto md:left-[-8px] md:right-auto md:top-1/2 md:-translate-y-1/2 md:border-b-transparent md:border-r-[#2e3868] md:border-l-transparent"></div>
                      </div>
                    )}
                  </div>
                </div>
                    ...
                    <div className="flex gap-2 mb-4 p-1 bg-gray-100 rounded-xl">
                      <button
                        ...
                        className={`flex-1 py-2.5 px-4 rounded-lg text-sm font-bold transition-all duration-200 ${
                          selectedSizeRange === 'mens' ? 'bg-[#2e3868] text-white shadow-sm' : 'text-gray-500 hover:text-gray-700'
                        }`}
                      >
                        Men&apos;s Sizing
                      </button>
                      <button
                        ...
                        className={`flex-1 py-2.5 px-4 rounded-lg text-sm font-bold transition-all duration-200 ${
                          selectedSizeRange === 'womens' ? 'bg-[#2e3868] text-white shadow-sm' : 'text-gray-500 hover:text-gray-700'
                        }`}
                      >
                        Women&apos;s Sizing
                      </button>
                    ...
                    {(selectedSizeRange === 'womens' ? parsedWomensSizes : parsedMensSizes).map((size) => (
                      <button
                        ...
                        className={`py-3 px-2 text-sm font-semibold rounded-xl border-2 transition-all duration-200 ${
                          selectedSize === size
                            ? 'bg-[#2e3868] border-[#2e3868] text-white shadow-md transform scale-[1.02]'
                            : sizeError
                  ...
                  <>
                    ...
                    {(product.checkoutFlow === 'paypal-invoice' || product.checkoutFlow === 'paypal-unclaimed' || product.checkoutFlow === 'paypal-direct' || product.checkoutFlow === 'paypal-api') ? (
                      <div className="hidden lg:flex flex-col gap-1.5">
                        ...
                        >
                          {isBuyingNow ? (
                            <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-[#2e3868]" />
                          ) : (
                            <Image
                ...
                <div className="rounded-[20px] border border-[#E8EEF8] bg-white px-5 py-5">
                  ...
                  {shouldCollapseDescription && (
                    <button
                      ...
                      className="mt-4 text-sm font-semibold text-[#2e3868] transition hover:text-[#1f274a]"
                    >
            ...
            <section className="rounded-[24px] border border-[#DCE5F5] bg-white px-8 py-8">
              ...
              {shouldCollapseDescription && (
                <button
                  type="button"
                  onClick={() => setShowFullDescription((current) => !current)}
                  className="mt-5 text-sm font-semibold text-[#2e3868] transition hover:text-[#1f274a]"
                >
                  {showFullDescription ? "Show less" : "Show more"}
                </button>
              )}
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        <span className="mt-0.5 flex-shrink-0 text-[#2e3868]" aria-hidden="true">
                      ...
                      {isOpen && (
                        <div className="pt-3 text-sm leading-7 text-[#5B6785]">
                          ...
                          {item.linkHref && item.linkLabel && (
                            <Link
                              href={item.linkHref}
                              className="mt-2 inline-flex text-sm font-semibold text-[#2e3868] transition hover:text-[#1f274a]"
                            >
                              {item.linkLabel}
                            </Link>
                          )}
              ...
              {faqItems.length > 4 && (
                <div className="border-t border-[#E8EEF8] px-6 py-5 sm:px-8">
                  <button
                    ...
                    className="text-sm font-semibold text-[#2e3868] transition hover:text-[#1f274a]"
                  >
        ...
        <div className="fixed inset-0 bg-black bg-opacity-90 z-50" onClick={() => setShowZoom(false)}>
          ...
          <div className="absolute inset-0 flex items-center justify-center p-4" onClick={(e) => e.stopPropagation()} onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
            <div className="relative w-full h-full">
              ...
              {images.length > 1 && (
                <>
                  <button onClick={(e) => { e.stopPropagation(); setActiveImage((prev) => (prev > 0 ? prev - 1 : images.length - 1)); setZoomLevel(1); }} className="absolute left-4 top-1/2 -translate-y-1/2 transform bg-white/10 hover:bg-[#2e3868] p-3 rounded-full text-white transition-colors duration-200" aria-label="Previous image"><ChevronLeft className="h-8 w-8" /></button>
                  <button onClick={(e) => { e.stopPropagation(); setActiveImage((prev) => (prev < images.length - 1 ? prev + 1 : 0)); setZoomLevel(1); }} className="absolute right-4 top-1/2 -translate-y-1/2 transform bg-white/10 hover:bg-[#2e3868] p-3 rounded-full text-white transition-colors duration-200" aria-label="Next image"><ChevronRight className="h-8 w-8" /></button>
                </>
              )}
            </div>
          </div>
