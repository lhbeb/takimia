...
export const metadata: Metadata = {
  ...
  description:
    'Official Takimia Shipping Policy. Free standard shipping across the United States. Same-day processing for orders placed before 2:00 PM EST.',
};
...
const policySections = [
  {
    title: 'Free Shipping (United States)',
    items: [
      'Free standard shipping on all orders across the United States',
      'No minimum purchase requirement',
      'Tracked shipping via premium logistics partners (USPS, FedEx, UPS)',
    ],
  },
  {
    ...
    items: [
      ...
      'Real-time tracking link with live carrier updates',
      'Delivery status notifications from warehouse to your doorstep',
    ],
  },
  {
    ...
    items: [
      ...
      'Residential and commercial addresses supported',
      'PO boxes and APO/FPO/DPO military addresses fully supported',
...
export default function ShippingPolicyPage() {
  const schemaMarkup = {
    ...
    '@graph': [
      {
        '@type': 'WebPage',
        '@id': 'https://takimia.com/shipping-policy',
        'url': 'https://takimia.com/shipping-policy',
        'name': 'Shipping Policy | Takimia',
        'description':
          'Takimia Shipping Policy: Free standard shipping across the United States. Same-day processing for orders placed before 2:00 PM EST.',
      },
      {
        '@type': 'OfferShippingDetails',
        '@id': 'https://takimia.com/shipping-policy#shipping-us',
        'shippingDestination': {
          '@type': 'DefinedRegion',
          'addressCountry': 'US',
        },
      ...
      <div className="container mx-auto max-w-5xl px-4">
        <section className="mb-10 rounded-2xl bg-[#2e3868] px-6 py-8 text-[#F0F6FF] sm:px-8 sm:py-10 shadow-lg">
          ...
          <p className="mt-5 max-w-3xl text-base leading-7 text-[#F0F6FF]/80 sm:text-lg">
            At Takimia, we focus on fast, reliable fulfillment with transparent delivery windows, free standard shipping across the United States, and real-time tracking from warehouse to door.
          </p>
        </section>

        <section className="mb-8 grid gap-4 md:grid-cols-3">
          <div className="rounded-xl border border-[#2e3868]/10 bg-white p-5 shadow-sm">
            <Clock className="mb-4 h-6 w-6 text-[#2e3868]" />
          ...
          <div className="rounded-xl border border-[#2e3868]/10 bg-white p-5 shadow-sm">
            <PackageCheck className="mb-4 h-6 w-6 text-[#2e3868]" />
          ...
          <div className="rounded-xl border border-[#2e3868]/10 bg-white p-5 shadow-sm">
            <ShieldCheck className="mb-4 h-6 w-6 text-[#2e3868]" />
        ...
        <section className="mb-8 rounded-2xl border border-[#2e3868]/10 bg-white p-6 shadow-sm sm:p-8">
              ...
              <ul className="mt-4 space-y-3 text-sm leading-6 text-gray-600">
                {section.items.map((item) => (
                  <li key={item} className="flex gap-3">
                    <span className="mt-2 h-1.5 w-1.5 flex-shrink-0 rounded-full bg-[#2e3868]" />
                    <span>{item}</span>
                  </li>
                ))}
              </ul>
        ...
        <section className="mt-8 rounded-2xl border border-[#2e3868]/10 bg-white p-6 shadow-sm sm:p-8">
          ...
          <Link
            href="/contact"
            className="mt-6 inline-flex items-center justify-center rounded-xl bg-[#2e3868] px-5 py-3 text-sm font-semibold text-[#F0F6FF] transition hover:bg-[#1f274a]"
          >
            Contact Support
          </Link>
        </section>
