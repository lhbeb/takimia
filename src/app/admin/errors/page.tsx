...
const TYPE_COLORS: Record<string, string> = {
  client: 'bg-[#2e3868]/10 text-[#2e3868] border-[#2e3868]/20',
        ...
        <div className="flex items-center gap-1.5 bg-white border border-gray-200 rounded-xl p-1">
          {['all', 'client', 'api', 'server'].map(t => (
            <button
              ...
              className={`px-3 py-1.5 text-xs font-medium rounded-lg transition-colors ${filterType === t ? 'bg-[#2e3868] text-white' : 'text-gray-600 hover:bg-gray-100'}`}
            >
        ...
        <div className="flex items-center gap-1.5 bg-white border border-gray-200 rounded-xl p-1">
          {[['unresolved', 'Unresolved'], ['resolved', 'Resolved'], ['all', 'All']].map(([val, label]) => (
            <button
              ...
              className={`px-3 py-1.5 text-xs font-medium rounded-lg transition-colors ${filterResolved === val ? 'bg-[#2e3868] text-white' : 'text-gray-600 hover:bg-gray-100'}`}
            >
              {label}
            </button>
          ))}
              ...
              >
                ...
                <div className="flex items-center gap-1 flex-shrink-0" onClick={e => e.stopPropagation()}>
                  <button
                    ...
                    className={`p-1.5 rounded-lg transition-colors ${log.resolved ? 'hover:bg-[#2e3868]/10 text-[#2e3868]' : 'hover:bg-green-50 text-green-500'}`}
                  >
                  ...
                  {!log.resolved && (
                    <div className="flex items-center gap-2 pt-1">
                      <input
                        ...
                        className="flex-1 text-xs px-3 py-2 border border-gray-200 rounded-lg focus:outline-none focus:border-[#2e3868]"
                      />
                      <button
