  ...
  return (
    <FixedCheckoutBar>
      <button
        ...
        onClick={onClick}
        disabled={disabled}
        className={`w-full font-bold py-4 px-6 rounded-xl transition-all duration-200 flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus:ring-4 focus:ring-[#2e3868] focus:ring-offset-2 text-lg sm:text-xl ${
          disabled
            ? 'bg-gray-400 cursor-not-allowed text-white'
            : 'bg-[#2e3868] hover:bg-[#1f274a] text-white active:scale-[0.98]'
        }`}
      >
        {isLoading ? (
          <>
        ...
        return (
          <button
            ...
            className={`w-full text-left p-3 hover:bg-blue-50 border-b border-gray-100 last:border-b-0 transition-colors duration-200 ${
              isSelected ? 'bg-blue-50 text-[#2e3868]' : 'text-[#262626]'
            }`}
...
function AddressFields({
  ...
  const inputClassName = `w-full px-4 py-4 border-2 border-gray-200 ${inputRadius} focus:outline-none focus:ring-2 focus:ring-[#2e3868] focus:border-[#2e3868] transition-all duration-300`;
          ...
          mobile
            ...
            : `w-full px-4 py-4 border-2 rounded-lg focus:outline-none focus:ring-2 transition-all duration-300 ${
                form.emailError
                  ...
                  : 'border-gray-200 focus:ring-[#2e3868] focus:border-[#2e3868]'
              }`
        }
...
function ContinueButton({
  ...
  return (
    <button
      ...
      disabled={isBusy}
      className={`w-full font-bold py-5 px-8 rounded-xl transition-colors duration-200 flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed text-white focus:outline-none focus:ring-4 focus:ring-[#2e3868] focus:ring-offset-2 text-xl ${
        isBusy ? 'bg-gray-400 cursor-not-allowed' : 'bg-[#2e3868] hover:bg-[#1f274a]'
      }`}
    >
      {isBusy ? (
        <>
          ...
          <span className="text-xl font-bold">
            {isSendingEmail ? 'Verifying Address...' : 'Redirecting...'}
          </span>
        </>
      ) : (
        <span className="text-xl font-bold">Verify Address</span>
      )}
    </button>
  );
}
  ...
  return (
    <div className={`${mobile ? 'lg:hidden mt-4 mb-4 space-y-2' : 'hidden lg:block mt-8 space-y-4'} flex flex-col items-center justify-center text-center w-full`}>
      <div className="text-sm text-gray-600">
        <span className="font-medium text-[#2e3868]">Secure Checkout</span> - SSL Encrypted
      </div>
      ...
      <div className={`flex flex-wrap items-center justify-center text-xs text-gray-500 mt-2 ${mobile ? 'gap-2 px-4' : 'gap-3'}`}>
        ...
        <Link href="/return-policy" className="hover:text-[#2e3868] hover:underline transition-colors">
          Refund and Return Policy
        </Link>
        <span className="text-gray-300">•</span>
        <Link href="/shipping-policy" className="hover:text-[#2e3868] hover:underline transition-colors">
          Shipping Policy
        </Link>
      </div>
    </div>
...
function formatVerifiedAddress(form: CheckoutFormController) {
  const parts = [
    form.shippingData.streetAddress,
    form.shippingData.addressLine2,
    form.shippingData.city,
    form.shippingData.state,
    form.shippingData.zipCode,
    form.shippingData.country,
  ].filter(Boolean);

  return parts.join(', ');
}

function AddressVerifiedNotice({ form }: { form: CheckoutFormController }) {
  return (
    <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800" role="status">
      <div className="flex items-start gap-3">
        ...
        <div>
          <p className="font-bold">Address verified</p>
          <p className="mt-1 leading-6">Address verified at {formatVerifiedAddress(form)}.</p>
          <p className="mt-2 text-xs font-medium text-emerald-700">Your order notice was saved and sent. Complete payment securely below.</p>
        </div>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen flex flex-col bg-gray-50 pb-40 lg:pb-4">
      ...
      <main className="flex-grow py-4">
        <div className="container mx-auto px-4">
          <Link href={`/products/${product.slug}`} className="inline-flex items-center text-[#2e3868] hover:text-[#1f274a] mb-4 text-sm">
              ...
              >
                <div className="flex items-center space-x-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[#2e3868] font-bold text-xl mb-1">{price}</p>
                    {sellerName && (
              ...
              {showMobileOrderSummary && (
                <div className="px-4 pb-4 border-t border-gray-100 mt-4 pt-4 space-y-4">
                  ...
                  <div className="flex justify-between items-center text-sm">
                    ...
                    <span className="font-medium text-[#2e3868]">Free</span>
                  </div>
                  <div className="border-t border-gray-200 pt-3 flex justify-between items-center">
                    <span className="text-base font-semibold text-[#262626]">Total</span>
                    <span className="text-lg font-bold text-[#2e3868]">{price}</span>
                  </div>
                </div>
              )}
            </div>
                  ...
                  <form onSubmit={onSubmit} className="space-y-6">
                    ...
                    {checkoutError && (
                      <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-800" role="alert">
                        ...
                        <button type="button" onClick={onDismissCheckoutError} className="mt-2 text-xs font-medium text-red-700 underline">Dismiss</button>
                      </div>
                    )}
              ...
              <div className="w-96 flex-shrink-0">
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <div className="flex-1 min-w-0 flex flex-col gap-2">
                      ...
                      <div className="flex flex-wrap items-center gap-1.5 mt-0.5">
                        ...
                        {(product as ProductWithSelectedSize).selectedSize && (
                          <span className="bg-[#2e3868]/8 text-[#2e3868] text-xs font-semibold px-2.5 py-1 rounded-full">
                  ...
                  <div className="px-6 pb-5 flex items-center justify-between">
                    <span className="text-xl font-bold text-[#2e3868]">{price}</span>
                    <button
                      type="button"
                      onClick={onClearCart}
              ...
              <form onSubmit={onSubmit} className="space-y-6">
                ...
                {product.checkoutFlow !== 'paypal-direct' && product.checkoutFlow !== 'paypal-api' && (!stripeClientSecret || !isAddressVerified) && (
                  <MobileCheckoutCTA
                    ...
                    loadingLabel={isSendingEmail ? 'Verifying Address...' : 'Redirecting...'}
