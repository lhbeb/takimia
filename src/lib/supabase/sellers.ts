...
function isCoffeeProductText(value: unknown): boolean {
  return typeof value === 'string' && /\b(coffee|espresso|grinder|barista|brewer|maker|cappuccino|latte|bean)\b/i.test(value);
}

function isCoffeeReview(review: Review): boolean {
  return isCoffeeProductText(review.productTitle) || isCoffeeProductText(review.productSlug);
}
...
function shouldIncludeNativeSellerReview(review: Review): boolean {
  ...
  // are explicitly tagged to a non-coffee legacy product.
  return !hasProductReference || isCoffeeReview(review);
}
    ...
    const publishedProductReviews = productRows
      ...
      .filter(
        (product) =>
          ...
          ([product.title, product.slug, product.category].some(isCoffeeProductText) || true),
      )
      .flatMap((product) =>
