  ...
  useEffect(() => {
    ...
    const loadStripeConfig = async () => {
      try {
        ...
        setConfigError('Payment is temporarily unavailable. Please email contact@takimia.com.');
      }
    };
        ...
        <div className="border-b border-gray-100 p-5 sm:p-8">
          {onBack && (
            <button
              type="button"
              onClick={onBack}
              className="mb-5 inline-flex items-center gap-2 text-sm font-medium text-[#2e3868] transition-colors hover:text-[#1f274a]"
            >
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <div className="rounded-2xl border border-blue-100 bg-blue-50 p-5">
              <div className="mb-2 flex items-center gap-2">
                ...
                <span className="font-semibold text-[#2e3868]">Confirmed Delivery Address</span>
              </div>
              ...
              {shippingData.email && (
                <div className="mt-3 flex items-center gap-2 text-[#2e3868]">
            ...
            <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
              ...
              {sellerName && (
                <div className="mt-1 text-sm text-gray-500">
                  Sold by: <span className="font-medium text-[#2e3868]">{sellerName}</span>
                </div>
              )}
              <div className="mt-5 flex items-end justify-between border-t border-gray-100 pt-4">
                <span className="text-sm font-semibold text-gray-500">Total</span>
                <span className="text-2xl font-extrabold text-[#2e3868]">{formattedPrice}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="min-h-[640px] p-3 sm:p-8">
          {configError ? (
            ...
            <div className="flex min-h-[420px] flex-col items-center justify-center">
              <div className="mb-4 h-12 w-12 animate-spin rounded-full border-4 border-[#2e3868]/25 border-t-[#2e3868]" />
