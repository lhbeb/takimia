            ...
            <div>
              <h2 className="text-2xl font-bold text-[#262626] mb-2">
                {sellerName && sellerUsername ? (
                  ...
                  'Reviews from this listing or similar other listings on Takimia'
                )}
              </h2>
                ...
                <div>
                  <div className="flex">
                    {[...Array(5)].map((_, i) => (
                      <Star
                        key={i}
                        className={`h-5 w-5 ${i < Math.floor(averageRating) ? 'text-[#2e3868] fill-[#2e3868]' : 'text-gray-300'}`}
                      />
                    ))}
                  </div>
              ...
              {Object.entries(ratingDistribution).reverse().map(([rating, percentage]) => (
                <div key={rating} className="flex items-center gap-2 mb-2">
                  ...
                  <div className="flex-grow bg-gray-200 rounded-full h-2">
                    <div
                      className="bg-[#2e3868] rounded-full h-2"
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="relative">
              <select
                ...
                className="appearance-none bg-white border border-gray-300 rounded-lg px-4 py-2 pr-8 text-gray-700 focus:outline-none focus:ring-2 focus:ring-[#2e3868] focus:border-transparent"
              >
            ...
            <div key={`${review.id}-${index}`} className="p-6">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-grow">
                  ...
                  {review.productTitle && (
                    <div
                      className="inline-block text-xs text-[#2e3868]/70 hover:text-[#2e3868] hover:underline mb-2 cursor-pointer group"
                  ...
                  <div className="flex mb-2">
                    {[...Array(5)].map((_, i) => (
                      <Star
                        key={i}
                        className={`h-4 w-4 ${i < review.rating ? 'text-[#2e3868] fill-[#2e3868]' : 'text-gray-300'}`}
                      />
                    ))}
                  </div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {review.images.filter(img => typeof img === 'string' && img.length > 0).map((image, imgIndex) => (
                          <button
                            ...
                            className="relative group overflow-hidden rounded-lg border border-gray-200 hover:border-[#2e3868] transition-colors duration-200"
                          >
                            <Image
                              src={image}
