...
export default function SellerBadge({ sellerId, size = 'sm' }: SellerBadgeProps) {
  ...
  if (size === 'sm') {
    return (
      ...
      >
        ...
        <span className="text-[11px] font-medium text-gray-600 group-hover:text-[#2e3868] transition-colors">
          {displaySeller.name}
        </span>
        {isTakimia ? (
          <ShieldCheck className="w-3 h-3 flex-shrink-0 text-[#2e3868]/50" />
        ) : (
  ...
  return (
    ...
    >
      {/* Avatar / icon */}
      <div className="w-5 h-5 rounded-full overflow-hidden bg-gray-100 flex items-center justify-center flex-shrink-0 ring-1 ring-gray-200 group-hover:ring-[#2e3868]/30 transition-all">
        {hasAvatar ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={displaySeller.avatarUrl} alt={displaySeller.name} className="w-full h-full object-cover" />
        ) : isTakimia ? (
          <ShieldCheck className="w-3 h-3 text-[#2e3868]" />
        ) : (
      ...
      <span className="text-sm font-medium text-gray-700 group-hover:text-[#2e3868] transition-colors">
        {displaySeller.name}
      </span>
      ...
      <div className="relative group flex items-center">
        {isTakimia ? (
          <ShieldCheck className="w-3.5 h-3.5 flex-shrink-0 text-[#2e3868]/50" />
        ) : (
          <Star className="w-3.5 h-3.5 flex-shrink-0 text-[#EFC154] fill-[#EFC154] cursor-help" />
        )}
        
        {/* Tooltip for md size only if not Takimia */}
        {!isTakimia && (
